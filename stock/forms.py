...
class AddBulkStockForm(forms.ModelForm):
    bulk_quantity = forms.IntegerField(min_value=1, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    bulk_expiry_date = forms.DateField(required=False, widget=forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}))
    bulk_batch = forms.CharField(required=False, max_length=50, widget=forms.TextInput(attrs={'class': 'form-control'}))

    class Meta:
        model = BulkStock
        fields = ['bulk_quantity', 'bulk_expiry_date', 'bulk_batch']
